...
facial_image = np.load(r"./facial_image3.npy")
facial_keypoints = np.load(r"./facial_keypoints3.npy")
...
dataset = image_keypoint_ds.repeat() # 重复数据集
dataset = dataset.shuffle(buffer_size = 10000) # 在缓冲区中随机打乱数据
batch   = dataset.batch(batch_size = 64) # 每10条数据为一个batch，生成一个新的Datasets

history = model.fit(batch, epochs=1000, steps_per_epoch=50)

model.save('model_keypoint12.h5')
tf.saved_model.save(model, 'saved_model_keypoint12') #可以给其余语言使用的

sample = iter(image_keypoint_ds)
s1 = next(sample)
print(s1)
