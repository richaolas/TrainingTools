...
def make_classification_dict(data_dir, image_name, text, label):
    # make the full url of the image sample
    image_path = os.path.join(data_dir, text, image_name)
    image = PIL.Image.open(image_path)
    data_dict = {
        'width': image.width,
        'height': image.height,
        'filename': os.path.join(text, image_name),  # the name url which relative to data dir
        # the object is the whole image
        'objects': [{
            'xmin': 0,
            'xmax': image.width,
            'ymin': 0,
...
def dict_to_tf_example(data_dict, dataset_directory):
    ...
    with tf.io.gfile.GFile(full_path, 'rb') as fid:
        ...
        # if need to crop the sub-image, then do the crop on the image
        if roi:
